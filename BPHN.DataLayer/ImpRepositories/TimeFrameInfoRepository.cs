    ...
    public class TimeFrameInfoRepository : BaseRepository, ITimeFrameInfoRepository
    {
        ...
        public async Task<List<TimeFrameInfo>> GetByPitchId(Guid pitchId)
        {
            using (var connection = ConnectDB(GetConnectionString()))
            {
                ...
                var lstTimeFrameInfo = await connection.QueryAsync<TimeFrameInfo>("select Id, SortOrder, Name, TimeBegin, TimeEnd, Price from time_frame_infos where PitchId = @pitchId", dic);
